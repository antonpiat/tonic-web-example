...
<template>
  <section :class="cardClass">
    <div class="flex w-full flex-col items-center gap-1 text-center">
      <h2 :class="headingClass">Math Service</h2>
      <div class="mb-1 flex w-full flex-wrap justify-center gap-2.5">
        <input
          type="number"
          v-model.number="num1"
          placeholder="Number 1"
          :class="[inputClass, 'w-[7.5rem]']"
        />
        <input
          type="number"
          v-model.number="num2"
          ...
          :class="[inputClass, 'w-[7.5rem]']"
        />
        <select v-model="operation" :class="inputClass">
      ...
      <button @click="doMath" :class="buttonClass">Calculate</button>
      <p :class="responseClass">Result: {{ mathResult }}</p>
    </div>
  </section>
</template>
